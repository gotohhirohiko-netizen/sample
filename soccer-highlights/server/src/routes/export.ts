...
exportRouter.post("/", upload.single("music"), (req: Request, res: Response) => {
...
async function runExportPipeline(
  ...
  for (let i = 0; i < uniqueVideoIds.length; i++) {
    ...
    const localPath = await ensureVideoDownloaded(source.youtubeUrl, videoId);
  ...
  await concatClips(clipPaths, concatenatedPath, jobTmpDir);
  ...
  if (musicBuffer) {
    ...
    await replaceAudioWithMusic(concatenatedPath, musicPath, finalPath);
  }
