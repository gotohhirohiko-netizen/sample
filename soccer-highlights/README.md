...
- プロジェクト名を付けてクリップ一覧を保存(`data/projects/`)。編集すると自動保存され、「今すぐ保存」ボタンも用意
...
1. 「プロジェクト」欄でプロジェクト名を入力する(例: 「8/10 練習試合」)。名前を付けると以降は
   クリップの追加・並べ替え・削除のたびに自動保存される
...
7. 「書き出し」でファイル名を入力し、必要なら差し替え用のmp3を選択して「書き出し開始」
   - 元動画のダウンロード → 各クリップの切り出し → 結合 → (mp3指定時)音声差し替え、の順に進み進捗が表示される
   - 完了すると`data/output/`にmp4として保存され、ダウンロードリンクが表示される
8. そのままYouTubeにアップロードする場合は、完了後に表示される「YouTubeにアップロード」パネルを使う
   (初回は認証設定が必要。下記参照)
...
soccer-highlights/
  ...
  data/                      実行時に生成(Git管理外)
    ...
    projects/                 保存したクリップ一覧(プロジェクト名ごとのJSON)
    tmp/                      書き出し処理中の一時ファイル
```
