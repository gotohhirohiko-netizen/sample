...
9. 「音楽を追加」より下の「既存の動画の音声を途中から差し替える」は、クリップ一覧や当時の音楽トラックの
   記録が残っていない古い書き出し済み動画(`data/output/`にあるものなら何でも)が対象。書き出し済みの
   動画ファイルを選び、差し替えたい時点(mm:ss)を指定すると、その時点より前はそのまま保持し、それ以降
   だけ現在「音楽を追加」欄に並んでいる曲に差し替えて新しいファイルとして書き出せる。複数の曲を1本の
   動画に適用したら最後の曲が途中で切れてしまった場合に、切れる手前で区切って別の曲を差し込み直す、
   といった用途に使う
