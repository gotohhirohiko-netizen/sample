...
- ダウンロードした元動画(`data/downloads/`)は書き出しジョブの中でその動画を使う最後のクリップの
  ...
  し直しになる点に注意。書き出しが途中で失敗・キャンセルされた場合も、その時点でダウンロード済みの
  元動画や一時ファイルは片付けられる。また、サーバーの強制終了などで片付けられなかった残骸があっても、
  次回`npm run dev` / `npm start`の起動時に`data/downloads/`・`data/tmp/`の中身は自動的に空にされる。
