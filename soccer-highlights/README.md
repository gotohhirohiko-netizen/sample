...
- ダウンロードした元動画(`data/downloads/`)は「1本ダウンロード→その動画のクリップを全て切り出す→
  即削除→次の動画」という順序で処理されるため、**同時にディスク上に残る元動画は基本的に常に1本だけ**
  (試合動画を数十本参照するような大きなプロジェクトでも、全部を先にダウンロードしてから削除する
  方式ではないため、必要なディスク容量は動画1本分程度で済む)。同じ動画から複数回に分けて書き出す
  場合は、その都度ダウンロードし直しになる点に注意。書き出しが途中で失敗・キャンセルされた場合も、
  その時点でダウンロード済みの元動画や一時ファイルは片付けられる。また、サーバーの強制終了などで
  片付けられなかった残骸があっても、次回`npm run dev` / `npm start`の起動時に`data/downloads/`・
  `data/tmp/`の中身は自動的に空にされる。
