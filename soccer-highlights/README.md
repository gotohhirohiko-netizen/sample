...
(一時的な回線の乱れなら再試行だけで直ることが多い)。このツールはダウンロード自体を自動で
最大3回リトライするが、それでも頻発する場合はWindowsでのIPv6経路の不安定さが原因のことが多いので、
`soccer-highlights/.env`に以下を追加してIPv4を強制してみること。
...
5. クリップ一覧で順序を↑↓(または移動先の番号を直接入力)で入れ替え、不要なものは削除、名前も編集可能。
   結合済み動画がある状態(書き出し済み)なら、行の「🔁 作り直す」で、そのクリップだけ動画を取得し直して
   結合済み動画内で差し替えられる(他のクリップは再ダウンロードしないため、全体をやり直すよりずっと速い。
   特定のクリップだけ画質が悪い・カクつく場合の修正に使う)。複数のクリップにチェックを入れて
   「選択したクリップを作り直す」を使うと、まとめて差し替えられる(同じ動画を参照するクリップを
   まとめて選べば、その動画のダウンロードは1回で済む。同じ動画由来のクリップは同じ問題が起きやすいため)
...
7. 「書き出し(切り抜き・結合)」でファイル名を入力して「結合して書き出し」
   - 元動画のダウンロード → 各クリップの切り出し → 結合、の順に進み進捗が表示される(この時点では
     音声は元のまま)
   - 処理が重い・想定より時間がかかる等の理由で途中でやめたい場合は「一時停止」を押すと、動画の
     ダウンロード完了時点・クリップの切り出し完了時点などキリのいいところで安全に停止する
     (実行中のダウンロード/エンコードを強制終了しない)。一時停止中はダウンロード済み・切り出し済みの
     ファイルがそのまま残り、後日サーバーを再起動した後でも「続きから再開」ボタンで途中から再開できる
   - すぐに中止したい場合は「キャンセル」を押す。その際「ダウンロード済みのファイルも削除するか」を
     確認される。「削除する」を選ぶと最初からやり直しになり、「保持する」を選ぶと一時停止と同様に
     あとで「続きから再開」できる
   - 完了すると`data/output/`にmp4として保存され、その結合済み動画がプロジェクトに記憶される。
     一時停止・キャンセル(保持)・書き出し失敗のいずれの場合も、途中経過は自動的にプロジェクトの
     作業フォルダに残るので、次にこのプロジェクトで書き出しを開こうとすると「前回の途中経過があります」
     と表示され、そこから再開するか、削除して最初からやり直すかを選べる
8. 音楽を追加したい場合は、その下の「音楽を追加」でmp3を選ぶ(複数選択・複数回の追加が可能。選んだ順に
   連結される)か、YouTube動画のURLを貼って音声だけを抽出して追加する。追加した音楽トラックは
   プロジェクトに保存されるため、ページを再読み込みしたり別のプロジェクトを開いてから戻ってきても
   消えない。「↑↓」で順序変更、「削除」で個別に外せる(削除は保存済みのファイルもサーバーから消える)。
   合計時間が動画の長さに対して足りているか不足しているかがその場で表示される(不足分は無音になる、
   超過分は自動的に切り詰められる)。一覧の各曲には「動画の◯◯時点まで再生」のように、その曲が動画の
   どの時間帯で流れるかも表示されるので、曲の途中で不自然に切れてしまう場合はどの曲を外すべきか判断
   しやすい。ファイル名を指定して「音声を適用して書き出し」を押すと、結合済み動画に対してだけ音声を
   差し替えるため、切り抜き・結合からのやり直しは不要で短時間で終わる。同じ結合済み動画に対して、
   別の曲の組み合わせで何度でも試すことができる
9. 「音楽を追加」より下の「既存の動画の音声を途中から差し替える」は、クリップ一覧や当時の音楽トラックの
   記録が残っていない古い書き出し済み動画(`data/output/`にあるものなら何でも)が対象。以下の3つを指定する。
   - **① 映像のベースにする動画ファイル**: 最終的な映像はこのファイルのものになる(クリップを追加して
     動画を作り直した場合は、その新しい動画を指定する)
   - **② 指定時点より前の音声を引き継ぐ動画ファイル(省略可)**: 省略すると①自身の音声を使う。①を新しく
     作り直したが、前半部分は以前選んだ音楽をそのまま引き継ぎたい場合、以前の(音楽が焼き込まれた)動画を
     指定する。②の映像は使われず、音声だけが使われる
   - **③ 差し替え時点(mm:ss)と、それ以降に使う曲**: 現在「音楽を追加」欄に並んでいる曲(その並び順)が
     使われる

   結果として、①の映像に対して、指定時点より前は②(省略時は①)の音声、それ以降は③の曲、をつなげた
   1本の動画ができる。複数の曲を1本の動画に適用したら最後の曲が途中で切れてしまった場合に、
   切れる手前で区切って別の曲を差し込み直す、といった用途に使う。「①のファイルを上書きする」に
   チェックを入れると、新規ファイルを作らず①をその場でこの結果に置き換える(元の①の内容は失われるが、
   ①と同じ映像を含む新しいファイルを別途作らずに済むためディスク使用量を抑えられる)。チェックしなければ
   従来通り別名の新規ファイルとして書き出される
10. そのままYouTubeにアップロードする場合は、各処理の完了後に表示される「YouTubeにアップロード」パネルを
    使う(初回は認証設定が必要。下記参照)
...
- プロジェクト名を変更すると、新しい名前で別ファイルとして保存される(元の名前のファイルはそのまま
  `data/projects/`に残る)。同じプロジェクトとして続けたい場合は名前を変えないこと。保存済みの
  音楽トラック(`data/projects/<プロジェクト名>.music/`)も同様に、名前を変えると新しい名前の
  プロジェクトには引き継がれない。
...
- ダウンロードした元動画は「1本ダウンロード→その動画のクリップを全て切り出す→即削除→次の動画」という
  順序で処理されるため、**同時にディスク上に残る元動画は基本的に常に1本だけ**(試合動画を数十本参照する
  ような大きなプロジェクトでも、全部を先にダウンロードしてから削除する方式ではないため、必要なディスク
  容量は動画1本分程度で済む)。この元動画は、一時停止・キャンセル(保持選択時)・失敗の際は
  `data/work/<プロジェクト名>/downloads/`に再開用として残るが、書き出しが最後まで成功すると片付けられる。
  同じ動画から複数のプロジェクトで書き出す場合は、その都度ダウンロードし直しになる点に注意。
- `data/tmp/`(音楽適用処理専用の一時領域)は、サーバーの強制終了などで片付けられなかった残骸があっても、
  次回`npm run dev` / `npm start`の起動時に自動的に空にされる。一方`data/work/`(一時停止・再開用の
  作業データ)は起動時には消さない(再開できなくなってしまうため)。書き出しを完全にやり直したい場合は、
  「結合して書き出し」を押した際に表示される確認ダイアログで「削除して最初からやり直す」を選ぶこと。
...
soccer-highlights/
  ...
  data/                      実行時に生成(Git管理外)
    work/                     プロジェクトごとの書き出し作業フォルダ(一時停止・再開用)
      <プロジェクト名>/downloads/  ダウンロード済み元動画(videoId単位、使い終わると自動削除)
      <プロジェクト名>/clips/      切り出し済みクリップ(clipId単位、結合成功で削除)
      <プロジェクト名>/meta.json   このジョブが処理しているクリップ一覧のスナップショット
    output/                   書き出し済みの結合済み動画・音声適用済み動画(ローカル保存先、自動削除されない)
    projects/                 保存したクリップ一覧・再生リスト・結合済み動画の参照(プロジェクト名ごとのJSON)
      <プロジェクト名>.music/     プロジェクトに追加した音楽トラックの実体(mp3、トラックID単位)
    tmp/                      音楽適用処理専用の一時ファイル(サーバー起動のたびに自動的に空になる)
```
